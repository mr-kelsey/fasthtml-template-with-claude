...
**Status: phases 0–9 complete** (see git history for full per-phase design rationale — each commit/phase weighed and rejected at least one simpler alternative on domain-modeling grounds, worth reading before revisiting that area). Implemented data model and features:
...
1. Quick wins: hide zero-on-hand seed/transplant lots from pickers (`quantity_on_hand` check in `farm/pages/inventory.py`); enlarge the bed resize-handle hit target (isolated CSS/JS, handle is a flat 1ft square regardless of zoom).
2. Frost-date-based planting validation — same pattern as the existing shade-tolerance warning; seasonally timely.
3. Task list (harden off seedlings, thin rows, start indoors) generated from planting dates — germination/harvest event infra already exists, extend it to generic tasks.
4. Weather/precipitation API ingestion — self-contained, no dependents, but unblocks frost-date accuracy and eventual dataset export.
5. Printable/exportable seasonal planting calendar.
6. Mobile-friendly bed entry — drag support exists, no responsive layout; bigger UI lift, not urgently wanted yet.
7. Cross-bed spacing/companion proximity — needs real map-space geometry between beds (not just one bed's grid); bigger lift, lower priority.
8. Photo attachment per bed — needs scoping against the existing variety reference-photo feature (seedling ID vs. bed documentation) before starting.
